# Git
Git is an source version control system that helps us to track project changes.  
you can download and install it from [Git-SCM Website](https://git-scm.com) website based on your OS. 

**Simple process:**   
1. Adding/Modifying file
2. Adding file to stage area by `git add filename` command
3. Commiting stage area files by running `git commit -m "Message"`
...
You can add your changed files to staging area by `git add .` (will add all changed files to staging area) or `git add filename` (will change only files that you write instead of filename (multiple files separated by space are accepted)).  

If you wanted to commit your changes without adding them to staging area, you can use `git commit -a` command, then your default editor will popup and you can set a commit message for it.  

You can see the commits list by using `git log` command.  
`git log -p` will show each commits changes one by one in separated pages.  
`git log --oneline` will show logs of commits and each commit in one line.  
`git log --graph` will show commits log in shape of a graph, while you are using branches this will help you to graphically separate each branch commits.  
...
**Pulling out files from stagin area**:  
1. `git rm --cached filename` this will pull out only the file that you write instead of filename from staging area.
2. `git rm --cached -rm .` this will pull out all files from staging area.
...
- You add your changed files to staging area to get prepared for commit by running `git add filename` or `git add -A` or `git add .` command.
- You commit your staged files and save them in local repository by running `git commit -m "Your Message"` command.
- You push your commits to remote repository by running `git push -u ...` command.
- You pull your codes from remote repository into your workspace by running `git pull` command.
- You pull your files from remote repository into local repository by calling `git fetch` command.

**Files will have 4 modes as bellow:**
![File Modes](../../assets/Git/FileModes.png)  

- Newly created files are Untracked files.
- Files with no changes since last commit are Unmodified files.
- Files that you have change them since your last commit are Modified files.
- Files that are in staging area that are prepared for commit those are Staged files.

`git checkout -- .`: Imagine you have some files in staging area and before commiting them, you made some change on those files, now you have some staged files that contains new changes, now if you run the `git checkout -- .` those changes that were implemented before `git add .` will remain but those changes that you have made after adding them to staging area will revert.  
...
`git reset —mixed ID` will change repository and staging area, it wouldn’t affect on working directory.  
...
- This one means that after running this command, you will reset your all changes, all areas to your last commit, you wouldn't have any changes on any areas after your last commit, so be careful about using this one!  
...
`git branch -d branchName` will delete the branch (safe delete, means if you had unmerged changes, it will not delete the branch).  
`git branch -d branchName` will delete the branch (Force delete, this will change your branch wether you have unmerged changes or not).  
